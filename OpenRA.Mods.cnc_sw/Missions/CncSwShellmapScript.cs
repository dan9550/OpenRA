...
{
	class CncSwShellmapScriptInfo : TraitInfo<CncSwShellmapScript> { }

	class CncSwShellmapScript: IWorldLoaded, ITick
	{
		...
		void SetViewport()
		{
			...
			var loc = ViewportOrigin.ToFloat2() + (new float2(-6,10) * float2.FromAngle( (float)t ));
		...
		public void Tick(Actor self)
		{
			...
			if (ticks == 100)
			{
                ...
                //Actors["Actor138"], new Order() { TargetLocation = new CPos(95, 49) });
                //Actors["Actor177"].Destroy().AttackTarget(Target.FromActor(Actors["Actor138"]), false, true);
			}
